...
将源论文（本组 E-taxi dual-stage RL, IEEE T-ASE 2025）思想**迁移适配到 E-bus**，并探索"双 PPO 联合"解耦方案与之对比。最终结论：**dual-stage（单多头 PPO + 启发式）在 E-bus 同质车队场景下优于双 PPO 联合**。
...
### 方法 A：dual-stage（单多头 PPO + 启发式，借鉴 E-taxi 前作）
- 单多头 PPO（发车 2 头 + 充电 1 头），同状态一次决策；
- 启发式执行：电量最高发车、电量最低充电。

### 方法 B：双 PPO 联合（本工作探索的解耦方案，集中式 critic）
...
dual-stage 全面更优。详见 `docs/成果总结.md`。
