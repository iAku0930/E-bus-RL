...
在 E-bus **同质车队**场景下，对比两种方法：**dual-stage（单多头 PPO 决策时机/数量 + 启发式选车，借鉴 E-taxi 前作）** 与 **双 PPO 联合智能体（本工作探索的解耦方案）**。结果表明 dual-stage 在各项指标上均更优：

| 方法 | 日均服务量 SP | 日均充电花费 CE | 日均总收益 | 低谷充电占比 |
|---|---|---|---|---|
| 双 PPO 联合（解耦） | 848 | 1015 | 768 | 65% |
| **dual-stage（单多头+启发式）** | **861** | **766** | **1084** | **91%** |
...
- **dual-stage（方法A）**：单多头 PPO 同状态决策"发车时机/数量 + 充电时机/数量"，启发式按电量排序选具体车。
- **双 PPO 联合（方法B）**：发车 actor + 充电 actor 解耦耦合，集中式 critic。
